...
public class DoorScriptBoss : MonoBehaviour
{
    ...
    void Update()
    {
        if (playerDetected)
        {
            if (Input.GetKeyDown(KeyCode.E) && enemySpawnerBoss.ableToTeleport)
            {
                enemySpawnerBoss.ableToTeleport = false;
    ...
    {
        if (col.CompareTag("Player") && enemySpawnerBoss.ableToTeleport)
        {
            playerDetected = true;
            playerGO = col.gameObject;
            keyTxt.SetActive(true);
            enemySpawnerBoss.SpawnEnemy();
        }
    }
