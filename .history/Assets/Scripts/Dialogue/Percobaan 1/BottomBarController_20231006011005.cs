...
public class BottomBarController : MonoBehaviour
{
    ...
    public void PlayNextSentence()
    {
        ...
        if(sentenceIndex % 2 != 0)
        {
            ...
            Image imageComponent = thuggerGameObject.GetComponent<Image>();
            tmp.a = 0.5f;
            npcPrefab.GetComponent<SpriteRenderer>().color = tmp;
        }
        else
        {
            // Instantiate(speaker.characterPrefab, celticPosition.transform.position, Quaternion.identity);
            Color tmp = CelticPrefab.GetComponent<SpriteRenderer>().color;
            tmp.a = 0.5f;
            CelticPrefab.GetComponent<SpriteRenderer>().color = tmp;
        }
    }
