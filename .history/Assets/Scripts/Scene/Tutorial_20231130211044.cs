...
public class Tutorial : MonoBehaviour
{
    ...
    void SafeZoneTutorial()
    {
        ...
        if(!slide1Done && !DialogueManager.instance.dialogueIsPlaying)
        {
            ...
            {
                wImage.color = new Color(wImage.color.r, wImage.color.g, wImage.color.b, 1);
                isWPressed = true;
            }

            if(Input.GetKeyDown(KeyCode.A))
            {
                aImage.color = new Color(aImage.color.r, aImage.color.g, aImage.color.b, 1);
                isAPressed = true;
            }

            if(Input.GetKeyDown(KeyCode.S))
            {
                sImage.color = new Color(sImage.color.r, sImage.color.g, sImage.color.b, 1);
                isSPressed = true;
            }

            if(Input.GetKeyDown(KeyCode.D))
            {
                dImage.color = new Color(dImage.color.r, dImage.color.g, wImage.color.b, 1);
                isDPressed = true;
            }
