...
public class PlayerStats : MonoBehaviour, IDataPersistence
{
    ...
    public void IncreaseLevel(GameData data)
    {
        int goldToConvert = data.gold;
        ...
        if (goldToConvert > 0)
        {
            ...
            data.gold -= goldToConvert;

            // Update the remaining gold for the current level
            remainingGoldForCurrentLevel = data.gold;
        }

        LevelUpChecker();
    }

    public void LevelUpChecker(GameData data)
    {
        if (data.experience >= experienceCap)
        {
            level++;
